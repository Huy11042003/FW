...
int ANCHOR_REFRESH_COUNT_set=15;

int finish_measure = 0;
...
void Tag_Measure_Dis(void)
{
    ...
    for(dest_anthor = 0 ;  dest_anthor<MAX_ANCHOR; dest_anthor++)
    {
            ...
            if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (status_reg & SYS_STATUS_RXFCG)
                {
                    ...
                    if (memcmp(rx_buffer, distance_msg, ALL_MSG_COMMON_LEN) == 0)
                    {
                        ...
                        Anthordistance[rx_buffer[12]] +=(int)(final_distance*1000);
...
if(UserSetNow.ANCHOR_TAG==0)
{
	...
	{
		UserSetNow.ID=buff[1];
        UserSetNow.MASTER=buff[2];
		TAG_ID=UserSetNow.ID;
		MASTER_TAG=UserSetNow.MASTER;
	}
	
	printf("device:TAG ID:%d",UserSetNow.ID);
    printf(" MASTER TAG ID:%d\r\n",UserSetNow.MASTER);
...
static void distance_mange(void)
{
    ...
    // compute_angle_send_to_anthor0(Anthordistance[0]/Anthordistance_count[0], Anthordistance[1]/Anthordistance_count[1],Anthordistance[2]/Anthordistance_count[2]);
    //printf("Anthor0distance:%3.2f\r\n", (float)Anthordistance[0]/1000);
	//printf("Anthor0distance_count:%d\r\n", Anthordistance_count[0]);
